...
const new_service = 'events_mp_ios'
const time2040 = 2536696935;

export class MPIOSNativeTransformer implements BusBaseEnricher {
  ...
  handle = async (key: string, msg: IncomingMessage): Promise<BaseIncomingMessageWithBatch> => {
    ...
    if (msg.data && msg.data.data && Array.isArray(msg.data.data)) {
      try {
        ...
        for (let [num, rec] of Object.entries(msg.data.data)) {
